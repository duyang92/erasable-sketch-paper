...
void testRFDSLinear() {
    ...
    fstream out;
    out.open("./Dist/Traffic/traffic_linear.txt", ios::out);
    ...
    for (int mem = (1 << startExp); mem <= (1 << endExp); mem <<= 1) {
        ...
        for (int rep = 1; rep <= repmax; rep++) {
            uint32_t _item = 1;
            RFDS_LINEAR rfdsLinearMap[MAX_REGION];
            srand((unsigned int) time(NULL));
            uint64_t startTime = timestamps[_item];
            ...
            for (int i = 0; i < MAX_REGION; i++) {
                ansMap[i] = 0;
                rfdsLinearMap[i].init(1.0, mem, 8, 0.8, random_seed1[rep], rand() % MAX_PRIME32, startTime, window, i);
            }
            ...
            while (curTime <= startTime + (TWS) * window && _item < MAXNUM) {
                ...
                for (int i = 1; i < MAX_REGION; ++i) {
                    rfdsLinearMap[i].insert(keyId, curTime, loc);
                }
                ...
                if (curTime - preTime >= window && curTime > preTime) {
                    for (int i = 1; i < MAX_REGION; ++i) {
                        ...
                        if (ansMap[i] == 0.0)
                            continue;
                        ss = fabs((double) rfdsLinearMap[i].query(curTime) / ansMap[i] - 1.0);
                        stds[i] += std::pow(ss, 2);
                        sums[i] += ss;
                        cnts[i] ++;
                        if (i == 1) {
                            out << curTime << " " << ansMap[i] << " " << rfdsLinearMap[i].query(curTime) << "\n";
                            //printf("query time %lld Region %d %d %.6lf %.6lf\t", curTime, i, rfdsLinearMap[i].query(curTime), ansMap[i], ss);
