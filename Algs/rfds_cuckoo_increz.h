...
class cuckoo_hash_table_increz {
   public:
    ...
    cuckoo_hash_table_increz(int size, double load_factor, int INDEX_SEED, uint64_t curTime) {
    ...
    int insert(uint32_t& key, uint64_t& curTime, uint64_t& outdatedTime) {
        ...
        bool su = _insert(key, curTime, outdatedTime);

        return su ? 0 : 2;
    }

    bool _insert(uint32_t& key, uint64_t& curTime, uint64_t& outdatedTime) {
        int attempts = 0;
...
#define C 240.0
   ...
   public:
    ...
    void init(double _alpha, double _memory, int _, double load_factor, uint32_t _FP_SEED, uint32_t _INDEX_SEED, uint64_t curTime, uint32_t window) {
        ...
        B = new cuckoo_hash_table_increz(max_elements, load_factor, INDEX_SEED, curTime);
    }
    ...
    void insert(uint32_t ele, uint64_t curTime) {
        ...
        if (flag == 1) {
            ...
            if (lz >= B->table2[B->bucket_count]) {
                ...
                while (flag == 2) {
                    cuckoo_hash_table_increz* tmp = new cuckoo_hash_table_increz(B->bucket_count * 2, B->load_factor, rand() % MAX_PRIME32, curTime);
                    tmp->table2[B->bucket_count] = B->table2[B->bucket_count];
                    for (int i = 0; i < B->bucket_count; ++i) {
                        if (B->table1[i] != B->EMPTY_BUCKET) {
                            flag = tmp->insert(B->table1[i], B->timestamp_slots1[i], outdatedTime);
                            if (flag == 2) {
                                break;
                            }
                        }
                        if (B->table2[i] != B->EMPTY_BUCKET) {
                            flag = tmp->insert(B->table2[i], B->timestamp_slots2[i], outdatedTime);
                            if (flag == 2) {
                                break;
                            }
                        }
                    }
                    if (flag == 0) flag = tmp->insert(fingerprint, curTime, outdatedTime);
                    if (flag == 0) {
                        delete B;
                        B = tmp;
                    }else{
