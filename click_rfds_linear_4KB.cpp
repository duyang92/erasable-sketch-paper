...
void testRFDSLinear(int startExp, int endExp, int _window = -1, int repmax = 1) {
    fstream out;
    out.open("./Dist/Click/click_rfds_linear.txt", ios::out);
    if (_window != -1)
        window = _window;
    ...
    for (int mem = (1 << startExp); mem <= (1 << endExp); mem <<= 1) {
        ...
        for (int rep = 1; rep <= repmax; rep++) {
            ...
            rfds_linear.init(1.0, mem, 8, 0.8, 13331, rand() % MAX_PRIME32, start_time, window);
            ...
            while (cur_time <= start_time + (TWS) * window && _item < flow_number) {
                //std::cout << "cur_time=" << cur_time << ",window=" << start_time + (TWS) * window << ",item=" << _item << ",flow number=" << flow_number << std::endl;
                rfds_linear.insert(flow[_item], cur_time);
                ...
                // show results (cur_time - start_time) % window == 0 && cur_time != start_time
                if (cur_time - pre_time >= window && cur_time && cur_time > pre_time) {
                    if (ans <=5) {
...
int main() {
    ...
    testRFDSLinear(start, end, 7200000, 50);
    return 0;
}
