...
void testRFDSCuckoo() {
    fstream out;
    out.open("./Dist/Traffic/traffic_cuckoo.txt", ios::out);
    ...
    for (int mem = (1 << startExp); mem <= (1 << endExp); mem <<= 1) {
        ...
        for (int rep = 1; rep <= repmax; rep++) {
            uint32_t _item = 1;
            RFDS_CUCKOO_INCREZ rfdsCuckooMap[MAX_REGION];
            srand((unsigned int) time(NULL));
            uint64_t startTime = timestamps[_item];
            ...
            for (int i = 0; i < MAX_REGION; i++) {
                ansMap[i] = 0;
                rfdsCuckooMap[i].init(1.0, mem, 8, 0.8, rand() % MAX_PRIME32, rand() % MAX_PRIME32, startTime, window, i);
            }
            ...
            while (curTime <= startTime + (TWS) * window && _item < MAXNUM) {
                ...
                for (int i = 1; i < MAX_REGION; ++i) {
                    rfdsCuckooMap[i].insert(keyId, curTime, loc);
                }
                ...
                if (curTime - preTime >= window && curTime > preTime) {
                    for (int i = 1; i < MAX_REGION; ++i) {
                        ...
                        if (ansMap[i] == 0.0)
                            continue;
                        ss = fabs((double) rfdsCuckooMap[i].query(curTime) / ansMap[i] - 1.0);
                        sums[i] += ss;
                        stds[i] += std::pow(ss, 2);
                        cnts[i] ++;
                        if (i == 1) {
                            out << curTime << " " << ansMap[i] << " " << rfdsCuckooMap[i].query(curTime) << "\n";
                            //printf("query time %lld Region %d %d %.6lf %.6lf\t", curTime, i, rfdsCuckooMap[i].query(curTime), ansMap[i], ss);
