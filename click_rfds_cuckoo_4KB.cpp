...
void testRFDSCuckoo(int startExp, int endExp, int _window = -1, int repmax = 1) {
    fstream out;
    out.open("./Dist/Click/click_rfds_cuckoo.txt", ios::out);
    if (_window != -1)
        window = _window;
    ...
    for (int mem = (1 << startExp); mem <= (1 << endExp); mem <<= 1) {
        ...
        for (int rep = 1; rep <= repmax; rep++) {
            ...
            rfds_cuckoo_increz.init(1.0, mem, 8, 0.8, 13331, rand() % MAX_PRIME32, start_time, window);
            ...
            while (cur_time <= start_time + (TWS) * window && _item < MAXNUM) {
                rfds_cuckoo_increz.insert(flow[_item], cur_time);
...
//                     printf("query time %lld\t%d\t%.6lf\t%.6lf\n", cur_time, rfds_cuckoo_increz.query(cur_time), ans, ss);
                    sum += ss;
...
int main() {
    ...
    testRFDSCuckoo(start, end, 7200000, 50);
