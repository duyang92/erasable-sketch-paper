...
class CDM : public Sketch{
   ...
   public:
    ...
    void init(double _1, double memory, int _bucket_slot, double _2, uint32_t _FP_SEED, uint32_t _INDEX_SEED, uint64_t curTime, uint32_t window, uint32_t _area) {
    ...
    void insert(uint32_t ele, uint64_t curTime, uint32_t locationNo) {
        //uint32_t curTime = static_cast<uint32_t>(curTime64 - startTime);
        ...
        for (int i = 0; i < bucket_slot; ++i) {
            if (buckets_slots[slot_index + i] >= guard_max) {
                ...
                timestamp_slots[slot_index + i] = curTime;
                //location_slots[slot_index + i] = locationNo;
                return;
        ...
        if (fingerprint < max_val) {
            ...
            //location_slots[max_slot] = locationNo;
