...
class double_hash_table {
   public:
    ...
    double_hash_table(int size, double load_factor, int _index_seed, uint64_t curTime, uint32_t _area) {
    ...
    bool insert(uint32_t& key, uint64_t curTime, uint64_t& outdatedTime, uint32_t areaNo) {
...
#define C 240.0
   ...
   public:
    ...
    void init(double _alpha, double _memory, int _, double load_factor, uint32_t _FP_SEED, uint32_t _INDEX_SEED, uint64_t curTime, uint32_t window, uint32_t areaNo) {
        alpha = _alpha;
        memory = _memory;
        area = areaNo;
        ...
        B = new double_hash_table(max_elements, load_factor, INDEX_SEED, curTime, area);
    }
